...
Cube Raceでは、プレイヤーを立方体として表示し、MediaPipeで取得した手や身体の位置をもとに移動・スケール変化を行っていました。また、Node.js / Express / Socket.IO によって2人の接続管理と位置同期を行っていました。
...
2人のプレイヤーがそれぞれのブラウザから接続し、Socket.IOによって位置や状態を同期しながらゴールを目指す対戦モードです。
...
MediaPipe Pose Landmarkerのランドマーク値を、ゲーム側で直接扱うのではなく、一度ゲーム用の入力値に変換します。
...
このように、身体ランドマークをゲームに必要な抽象的な入力値へ変換することで、処理を整理しやすくしています。
...
体の傾きは、左右の肩の高さ差から推定します。

```txt
leftShoulder.y - rightShoulder.y
```

この値をもとに、左傾き・右傾き・中央状態を判定します。

### Crouch

屈み動作は、キャリブレーション時の肩中心の高さと現在の肩中心の高さを比較して判定します。

```txt
currentShoulderY - baseShoulderY
```

画像座標では下方向に行くほど y が大きくなるため、基準姿勢より肩中心が下がった場合に屈み状態として扱います。

### Arm Swing

正面カメラでは腕の前後運動を直接検出しにくいため、本作品では腕振りを「左右の手首が交互に上下するリズム」として扱います。

```txt
leftWrist.y - rightWrist.y
```

この値が一定時間内に正負へ切り替わるかを見て、腕振りの強さを推定します。

### Ready Pose

ready状態は、両手首が頭より上にあるか、または顔の周辺にあるかをもとに判定します。
